################################################################################
# Simulation of Respiratory Syncytial Virus (RSV) Ct trajectories with residual error
# Author: Laura Mulas
...
df_sim <- data %>%
  ...
  mutate(
    ID = row_number(),
    periode_epi = as.character(periode_epi)
  )
...
for (chunk_start in chunk_starts) {
  ...
  df_long_sim_chunk <- df_traj_chunk %>%
    ...
    dplyr::select(sim, ID, ID_infection, age_cat, periode_epi, season,
  ...
  outfile_traj <- file.path(
    outdir,
    paste0("simulation_TV_VRS_", chunk_start, "_", chunk_end, "_error_traj_vfinal.rds")
  )
  ...
  outfile_peak <- file.path(
    outdir,
    paste0("simulation_TV_VRS_", chunk_start, "_", chunk_end, "_error_peak_vfinal.rds")
  )
