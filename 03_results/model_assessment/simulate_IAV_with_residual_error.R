################################################################################
# Simulation of Influenza A virus (IAV) Ct trajectories with residual error
# Author: Laura Mulas
...
popparam_txt <- "03_results_analysis/model_outputs/IAV/populationParameters.txt"

# Output directory
outdir <- "03_results_analysis/results/simulations_error/IAV"
...
df_sim <- data %>%
  distinct(ID_infection, age_cat, periode_epi, season) %>%
...
beta_p_season_25_26             <- df_params$value[df_params$parameter == "beta_p_season_25_26"]
beta_delta_age_fus_old_G__5     <- df_params$value[df_params$parameter == "beta_delta_age_fus_old_G__5"]
beta_delta_age_fus_old_G_5_18   <- df_params$value[df_params$parameter == "beta_delta_age_fus_old_G_5_18"]
beta_delta_season_25_26         <- df_params$value[df_params$parameter == "beta_delta_season_25_26"]
beta_delta_period_fus_G_Phase_1 <- df_params$value[df_params$parameter == "beta_delta_period_fus_G_Phase_1"]
...
for (chunk_start in chunk_starts) {
  ...
  for (i in chunk_start:chunk_end) {
    for (j in seq_len(n_ind)) {
      ...
      log_p_i <- log(p_pop) +
        beta_p_season_25_26 * (df_sim$season[j] == "25_26")
      ...
      log_delta_i <- log(delta_pop) +
        beta_delta_age_fus_old_G__5 * (df_sim$age_cat[j] == "<5") +
        beta_delta_age_fus_old_G_5_18 * (df_sim$age_cat[j] == "5-18") +
        beta_delta_season_25_26 * (df_sim$season[j] == "25_26") +
        beta_delta_period_fus_G_Phase_1 * (df_sim$periode_epi[j] == "Phase 1") +
        ranef_delta[j, i]
  ...
  df_long_sim_chunk <- df_traj_chunk %>%
    ...
    dplyr::select(sim, ID, ID_infection, age_cat, season, periode_epi,
  ...
  outfile_traj <- file.path(
    outdir,
    paste0("simulation_TV_flu_A_", chunk_start, "_", chunk_end, "_error_traj_vfinal.rds")
  )
  ...
  outfile_peak <- file.path(
    outdir,
    paste0("simulation_TV_flu_A_", chunk_start, "_", chunk_end, "_error_peak_vfinal.rds")
  )
