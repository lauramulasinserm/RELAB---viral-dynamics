################################################################################################
# Simulation of Respiratory Syncytial Virus (RSV) trajectories
# Author: Laura MULAS
# Purpose:
...
popparam_txt <- paste0(
  "03_results_analysis/model_outputs/RSV/",
...
outdir <- "03_results_analysis/results/simulations/RSV"
...
mu_full <- df_params$value[1:12]
names(mu_full) <- df_params$parameter[1:12]
...
# In the final RSV model, inter-individual variability is retained on beta.
omega_beta <- pmax(
  sim_df$omega_beta,
  1e-6
)
...
for (i in seq_len(n_sim)) {
  ...
  for (j in seq_len(n_ind)) {
    ...
    log_delta <-
      ...
      if (
        "beta_delta_age_cat__5" %in% names(theta_i) &&
          df_sim$age_cat[j] == "<5"
      ) {
        theta_i["beta_delta_age_cat__5"]
      } else {
        0
      } +

      if (
        "beta_delta_age_cat_5_18" %in% names(theta_i) &&
          df_sim$age_cat[j] == "5-18"
      ) {
        theta_i["beta_delta_age_cat_5_18"]
      } else {
        0
      } +

      if (
        "beta_delta_age_cat__65" %in% names(theta_i) &&
          df_sim$age_cat[j] == ">65"
      ) {
        theta_i["beta_delta_age_cat__65"]
      } else {
        0
      } +
...
message("RSV simulations completed.")
